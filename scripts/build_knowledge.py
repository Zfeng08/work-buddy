...
**双份输出（隐私隔离）**：
- `data/knowledge.json`      ← 只含 `knowledge/notes/`（公开笔记），提交入库、线上展示
- `data/knowledge.local.json` ← 额外并入 `knowledge/_private/`（私有笔记，.gitignore 排除），
  只在本地生成，供本机预览看到全部笔记。前端本地优先读它，线上文件不存在自动回退。

这样私有笔记永远不会进入 Git，也不会被自动化推到远端。
...
PRIVATE_DIR = NOTES_DIR / "_private"          # 本地私有笔记：.gitignore 排除，永不入库
DATA_PATH = ROOT / "data" / "knowledge.json"  # 公开版（入库、上线）
LOCAL_DATA_PATH = ROOT / "data" / "knowledge.local.json"  # 本地完整版（不入库）
...
def note_paths(include_private=False):
    """列出待解析的笔记文件。

    `knowledge/_private/` 是本地私有目录（.gitignore 排除），
    只有 include_private=True 时才纳入；公开构建永远看不到它。
    """
    out = []
    for p in sorted(NOTES_DIR.rglob("*.md")):
        if "_private" in p.parts and not include_private:
            continue
        if p.name == "README.md":
            continue
        out.append(p)
...
def load_notes(include_private=False):
    notes = []
    for path in note_paths(include_private):
...
def build_payload(notes):
    ...
    edges = build_relations(notes)  # notes 自带 _bigrams，无需重复扫盘
...
def main():
    # ① 公开版：只含 knowledge/notes/，提交入库、线上展示
    pub_notes = load_notes(include_private=False)
    if pub_notes:
        pub = build_payload(pub_notes)
        write_json(DATA_PATH, pub)
        s = pub["stats"]
        print(f"[ok] 公开版 → data/knowledge.json: {s['note_count']} 篇笔记, "
              f"{s['keyword_count']} 个关键词, {s['link_count']} 条关联, {s['tag_count']} 个标签")
    else:
        # 一篇公开笔记都没有时不要写出空文件，否则会覆盖掉线上已有的索引
        print("[skip] knowledge/notes/ 为空，保留现有 data/knowledge.json")

    # ② 本地完整版：并入 knowledge/_private/（.gitignore 排除，永不入库）
    #    Actions 上没有这个目录，所以 CI 里不会生成 —— 私有内容不会外泄。
    if PRIVATE_DIR.exists():
        all_notes = load_notes(include_private=True)
        full = build_payload(all_notes)
        write_json(LOCAL_DATA_PATH, full)
        s = full["stats"]
        extra = s["note_count"] - len(pub_notes)
        print(f"[ok] 本地完整版 → data/knowledge.local.json: {s['note_count']} 篇"
              f"（含私有 {extra} 篇）, {s['keyword_count']} 个关键词")
    else:
        print("[info] 无 knowledge/_private/，跳过本地完整版")
