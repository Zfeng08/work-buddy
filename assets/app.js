...
const state = {
  ...
  dramasPlatform: "全部", dramasState: "全部",
  openNoteId: null,
  kbBase: null,          // 公开版知识库（未解锁私有笔记时的基线）
};
...
/* ---------- 数据加载 ---------- */
const DATASETS = ["news", "finance", "weather", "jobs", "dramas", "trends", "knowledge"];

// 数据源优先级：
//   1) data/knowledge.local.json —— 本机构建的完整版（含私有笔记），线上没有此文件会自然跳过
//   2) raw.githubusercontent.com —— 仓库为 public，实时反映最新提交，不受 Pages 部署节奏影响
//   3) 同源相对路径             —— 兜底（本机预览、网络受限场景）
const RAW_BASE =
...
// 每个数据集按优先级列出候选地址，逐个尝试，先成功者胜出。
function dataCandidates(name) {
  const urls = [];
  if (name === "knowledge") urls.push("data/knowledge.local.json");
  if (IS_LOCAL) {
    urls.push(`data/${name}.json`, RAW_BASE + name + ".json");
  } else {
    urls.push(RAW_BASE + name + ".json", `data/${name}.json`);
  }
  return urls;
}
...
async function loadJSON(name) {
  for (const url of dataCandidates(name)) {
    try { return await fetchJSON(url); } catch (e) { /* 试下一个候选 */ }
  }
  return null;
}
...
async function loadAll(isRefresh = false) {
  ...
  state.data = next;
  state.kbBase = next.knowledge;   // 公开版基线，解锁私有笔记时的合并底本
  applyPrivateLock();   // 公开数据刚刷新，重新套用已解锁的私有笔记
...
/* ---------- 私有笔记解锁（浏览器端解密，明文不出本机） ----------

   私有笔记的明文只存在于本机，公开仓库里只有 data/private.enc.json 这份密文
   （PBKDF2-HMAC-SHA256 60 万次 + AES-256-GCM），由 scripts/build_private_bundle.py 生成。
   下面用 WebCrypto 做同一套算法，输口令 → 解密 → 并入公开知识库一起渲染。

   解出来的内容只放在 sessionStorage，关掉标签页即消失；页面不落任何明文到磁盘。
--------------------------------------------------------------------------- */
const LOCK_SS_KEY = "wb_private_notes";
...
async function decryptPrivate(passphrase) {
  if (!window.crypto?.subtle) {
    throw new Error("当前环境不支持 WebCrypto（需要 https 或 localhost）");
  }
  const blob = await loadJSON("private.enc");
  if (!blob?.ct) throw new Error("没找到 data/private.enc.json");

  const enc = new TextEncoder();
  // WebCrypto 要求哈希名带连字符（SHA-256）；密文里两种写法都兼容一下
  const hashName = String(blob.kdf.hash || "SHA-256").toUpperCase().replace(/^SHA(\d+)$/, "SHA-$1");
  const baseKey = await crypto.subtle.importKey(
    "raw", enc.encode(passphrase), "PBKDF2", false, ["deriveKey"]);
  const key = await crypto.subtle.deriveKey(
    {
      name: "PBKDF2",
      salt: b64ToBytes(blob.kdf.salt),
...
// 把私有笔记并进公开知识库，并在客户端重建关键词/标签索引与关联。
function mergeKnowledge(base, privNotes) {
  const notes = [...(base?.notes || []), ...privNotes];
  const kwIndex = {};
  notes.forEach((n) => (n.keywords || []).forEach((k) => {
    (kwIndex[k] = kwIndex[k] || []).push({ note_id: n.id, count: Math.max(1, countIn(n.body_html || "", k)) });
  }));
  // 与 build_knowledge.py 的口径一致：全局出现不足 2 次的词不入索引
  const keyword_index = {};
  Object.entries(kwIndex).forEach(([k, arr]) => {
    if (arr.reduce((s, x) => s + x.count, 0) >= 2) keyword_index[k] = arr;
  });

  const tag_index = {};
  notes.forEach((n) => (n.tags || []).forEach((t) =>
    (tag_index[t] = tag_index[t] || []).push(n.id)));

  const kwSets = notes.map((n) => new Set(n.keywords || []));
  const related = Object.fromEntries(notes.map((n) => [n.id, []]));
  for (let i = 0; i < notes.length; i++) {
    for (let j = i + 1; j < notes.length; j++) {
      const shared = [...kwSets[i]].filter((k) => kwSets[j].has(k));
      if (!shared.length) continue;
      const score = Math.round(shared.length / Math.min(kwSets[i].size, kwSets[j].size) * 100);
      related[notes[i].id].push({ id: notes[j].id, score, reasons: shared.slice(0, 3) });
      related[notes[j].id].push({ id: notes[i].id, score, reasons: shared.slice(0, 3) });
    }
  }
  Object.values(related).forEach((a) => a.sort((x, y) => y.score - x.score));
  ...
  return {
    ...
    stats: {
      ...
      tag_count: Object.keys(tag_index).length,
      link_count: Object.values(related).reduce((s, a) => s + a.length, 0),
    },
    private_count: privNotes.length,
  };
}

function applyPrivateLock() {
  const base = state.data.knowledge;
  if (!base || !base.notes) return;
  let cached = null;
  try { cached = JSON.parse(sessionStorage.getItem(LOCK_SS_KEY) || "null"); } catch { /* 忽略 */ }
  if (Array.isArray(cached) && cached.length) {
    state.data.knowledge = mergeKnowledge(base, cached);
  }
}

function setLockMsg(html, kind) {
  const el = $("#lock-msg");
  if (!el) return;
  el.innerHTML = html;
...
function refreshLockUI() {
  ...
  const n = kb?.private_count || 0;
  const form = $("#lock-form");
  if (form) form.hidden = n > 0;
  if ($("#lock-clear")) $("#lock-clear").hidden = n === 0;
  if (n > 0) {
    setLockMsg(`已解锁 <b>${n}</b> 篇私有笔记（仅本机标签页内可见，关掉即锁）`, "ok");
  } else if (!$("#lock-msg").classList.contains("err")) {
    setLockMsg("线上只有公开笔记。输入口令在本机解密，可显示私有笔记。");
  }
}
...
function initLockUI() {
  ...
  form.onsubmit = async (e) => {
    ...
    const btn = $("#lock-btn");
    btn.disabled = true;
    setLockMsg("正在解密（PBKDF2 60 万次，约 1 秒）…");
    try {
      const payload = await decryptPrivate(pass);
      const notes = payload.notes || [];
      sessionStorage.setItem(LOCK_SS_KEY, JSON.stringify(notes));
      state.data.knowledge = mergeKnowledge(state.kbBase || state.data.knowledge, notes);
      $("#lock-pass").value = "";
      refreshLockUI();
      renderKnowledge();
    } catch (err) {
      setLockMsg("口令不对，或密文文件读不到。", "err");
    } finally {
      btn.disabled = false;
    }
  };
  const clear = $("#lock-clear");
  if (clear) {
    clear.onclick = () => {
      sessionStorage.removeItem(LOCK_SS_KEY);
      if (state.kbBase) state.data.knowledge = state.kbBase;
      renderKnowledge();
      refreshLockUI();
    };
  }
...
function renderDramas() {
  const d = state.data.dramas;
  const grid = $("#dramas-grid");
  if (!grid) return;
  if (!d || !d.items) {
    grid.innerHTML = `<div class="empty">剧集数据尚未采集</div>`;
    return;
  }
  $("#dramas-meta").textContent =
    `${d.count} 部 · ${d.on_air_count} 部更新中 · ${d.rated_count} 部有豆瓣评分`;

  const platsMeta = d.platforms || {};
  const pValues = ["全部"].concat(Object.keys(platsMeta));
  const pCounts = { 全部: d.count };
  Object.entries(platsMeta).forEach(([id, p]) => { pCounts[id] = p.count; });
  bindChipRow($("#dramas-platform-filter"), pValues, state.dramasPlatform,
    (v) => { state.dramasPlatform = v; renderDramas(); },
    (v) => `${v === "全部" ? "全部" : (platsMeta[v]?.name || v)} ${pCounts[v] ?? 0}`);

  binderStateChips();

  let items = d.items.slice();
  if (state.dramasPlatform !== "全部") {
    items = items.filter((it) => (it.platforms || []).includes(state.dramasPlatform));
  }
  if (state.dramasState === "更新中") items = items.filter((it) => it.on_air);
  if (state.dramasState === "有评分") {
    items = items.filter((it) => (it.douban || {}).douban_rating);
  }

  grid.innerHTML = items.length
    ? items.map((it) => dramaCard(it, platsMeta)).join("")
    : `<div class="empty">没有符合条件的剧集</div>`;
}

function binderStateChips() {
  bindChipRow($("#dramas-state-filter"), ["全部", "更新中", "有评分"], state.dramasState,
    (v) => { state.dramasState = v; renderDramas(); });
}

function dramaCard(it, platsMeta) {
  const db = it.douban || {};
  const plats = (it.platforms || []).map((p) => {
    const m = platsMeta[p] || {};
    return `<span class="drama-plat" style="--pc:${m.color || "#888"}">${esc(m.name || p)}</span>`;
  }).join("");
  const cover = it.cover
    ? `<img class="drama-cover" src="${esc(it.cover)}" alt="" loading="lazy" referrerpolicy="no-referrer">`
    : `<div class="drama-cover drama-cover-ph">暂无海报</div>`;
  const rating = db.douban_rating
    ? `<span class="drama-rating">${db.douban_rating.toFixed(1)}</span>`
    : `<span class="drama-rating drama-rating-none">暂无</span>`;
  const extraTags = (it.categories || []).slice(0, 2)
    .concat(it.on_air ? ["更新中"] : [])
    .map((t) => `<span class="drama-tag">${esc(t)}</span>`).join("");
  const sub = [
    it.year ? `${it.year}` : "",
    (it.actors || []).slice(0, 2).join(" / "),
  ].filter(Boolean).join(" · ");
  const link = db.douban_url || it.url || "#";
  return `<a class="drama-card" href="${esc(link)}" target="_blank" rel="noopener">
    <div class="drama-poster">${cover}${rating}</div>
    <div class="drama-info">
      <div class="drama-title" title="${esc(it.title)}">${esc(it.title)}</div>
      <div class="drama-sub">${esc(sub || it.status || "")}</div>
      <div class="drama-tags">${plats}${extraTags}</div>
    </div>
  </a>`;
}
...
function renderAll() {
  ...
  renderJobs(); renderDramas(); renderTrends(); renderKnowledge(); renderGraph();
}
